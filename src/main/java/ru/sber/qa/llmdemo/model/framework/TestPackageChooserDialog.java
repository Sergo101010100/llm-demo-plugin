...
/**
 * Взято отсюда c доработкой для тестов {@link com.intellij.ide.util.PackageChooserDialog}
 * Изменение в методе {@link TestPackageChooserDialog#createTreeModel()}
 */
public class TestPackageChooserDialog extends PackageChooser {
    ...
    public TestPackageChooserDialog(@NlsContexts.DialogTitle @NotNull String title, @NotNull Project project,
    ...
    protected JComponent createCenterPanel() {
        ...
        myTree.getSelectionModel().addTreeSelectionListener(e -> updatePathFromTree());
    ...
    private ActionButton createNewPackageActionButton() {
        TestPackageChooserDialog.NewPackageAction newPackageAction = new TestPackageChooserDialog.NewPackageAction();
    ...
    static class PackageUpdate {
        ...
        private final @Nullable TestPackageChooserDialog.PackageUpdate myParentUpdate;

        PackageUpdate(@NotNull PsiPackage pkg) {
            myPkg = pkg;
            PsiPackage parentPkg = pkg.getParentPackage();
            myParentUpdate = parentPkg == null ? null : new TestPackageChooserDialog.PackageUpdate(parentPkg);
        }
        ...
        @Nullable
        TestPackageChooserDialog.PackageUpdate getParentUpdate() {
            return myParentUpdate;
        }
    }

    void createTreeModel() {
        List<TestPackageChooserDialog.PackageUpdate> pkgUpdates = ActionUtil.underModalProgress(
                myProject,
                JavaBundle.message("package.chooser.modal.progress.title"
                ), () -> {
                    PsiManager psiManager = PsiManager.getInstance(myProject);
                    //делаем ограничение по модулю
                    FileIndex fileIndex = ModuleRootManager.getInstance(myModule).getFileIndex();;
                    List<TestPackageChooserDialog.PackageUpdate> pkgUpdateList = new ArrayList<>();
                    fileIndex.iterateContent(fileOrDir -> {
                        //по сравнению с классом откуда взято - изменено тут
                        if (fileOrDir.isDirectory() && fileIndex.isUnderSourceRootOfType(fileOrDir, ContainerUtil.newHashSet(JavaSourceRootType.TEST_SOURCE))) {
                            ...
                            if (pkg != null) pkgUpdateList.add(new TestPackageChooserDialog.PackageUpdate(pkg));
                        }
                        return true;
                    });
                    return pkgUpdateList;
                });
        for (TestPackageChooserDialog.PackageUpdate pkgUpdate : pkgUpdates) addPackage(pkgUpdate);
    ...
    @NotNull
    private DefaultMutableTreeNode addPackage(@NotNull TestPackageChooserDialog.PackageUpdate pkgUpdate) {
        ...
        CommandProcessor.getInstance().executeCommand(myProject, () -> {
            Runnable action = () -> {
                try {
                    ...
                    if (newPackage != null) {
                        DefaultMutableTreeNode newChild = addPackage(new TestPackageChooserDialog.PackageUpdate(newPackage));
